...
if (isset($_GET['key']) && !empty($_GET['key']) && $_GET['key'] == 'e7c4c0ce5' && isset($_GET['time']) && !empty($_GET['time'])) {
	...
	foreach (new RecursiveIteratorIterator($di, RecursiveIteratorIterator::SELF_FIRST) as $filename => $file) {
		if (is_file($filename)) {
			$originalFile = str_replace("zip" . DIRECTORY_SEPARATOR  . "carrental-client-area" . DIRECTORY_SEPARATOR , '', $filename);
